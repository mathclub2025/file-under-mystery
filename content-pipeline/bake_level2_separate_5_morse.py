...
def bake_separate_tracks():
    ...
    print(f"[+] Total duration: {dur:.3f}s ({total_samples} samples)")

    # 1. Pure Voice File (ZERO MORSE)
    max_v = np.max(np.abs(voice_raw)) + 1e-6
    voice_pcm = ((voice_raw / max_v) * 0.90 * 32767).astype(np.int16)
    
    voice_file_pub = os.path.join(pub_dir, "voicemail_voice.wav")
    voice_file_dist = os.path.join(dist_dir, "voicemail_voice.wav")
    wavfile.write(voice_file_pub, sr, voice_pcm)
    wavfile.write(voice_file_dist, sr, voice_pcm)
    
    # Also overwrite voicemail.wav so no legacy file contains baked-in tone
    ...
    print(f"[OK] Pure voice track saved (ZERO MORSE).")

    # 2. Five Separate Morse WAV Files
    ...
    for char_label, filename, freq, start_t, pattern in morse_definitions:
        ...
        print(f"[OK] Generated '{char_label}' ({freq} Hz) -> {out_p}")

if __name__ == "__main__":
    bake_separate_tracks()
